...
const FASTLeaguesMenu = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchTeamsName = async () => {
      try {
        const response = await axios.get(`http://192.168.10.8:5001/getLeagueTeams?League_Name=${nname}`);
        if (response?.data) {
  ...
  useEffect(() => {
    const fetchMatchesSchedule = async () => {
      try {
        const response = await axios.get(`http://192.168.10.8:5001/getLeagueSchedule?League_Name=${nname}`);
        if (response?.data) {
